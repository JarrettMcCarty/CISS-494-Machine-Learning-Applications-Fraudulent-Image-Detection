...
def main():
    ...
    print('Available train sizes ...')
    print(train_sizes)
    print('Available cv sizes ...')
    print(cv_sizes)
    train_size = int(input("Train Size: "))
    cv_size = int(input("CV Size: "))

    use_full = True

    if (train_size not in train_sizes or cv_size not in cv_sizes):
    ...
    path = 'greyscalestride32/train_data/'



    #path_binary ='binarystride8/train_data/'
    #x_train_grayscale = np.load(path + 'x_train.npy')
    #x_cv_grayscale = np.load(path + 'x_cv.npy')

    #y_train_grayscale = np.load(path + 'y_train.npy')
    #y_cv_grayscale = np.load(path + 'y_cv.npy')
